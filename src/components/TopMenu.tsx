      ...
      <div className="flex flex-row items-center justify-between px-6 md:px-12 h-full">
        <div>
          <Link href="/" onClick={closeMenu} className="font-black text-2xl text-cyan-700 tracking-tighter hover:scale-105 transition-transform inline-block">
            <div className="flex items-center">
              <span className="text-2xl mr-2">💼</span> JobFair
            </div>
          </Link>
        </div>
        ...
        <div className="hidden md:flex flex-row items-center space-x-6 md:space-x-8">
          ...
          {session ? (
            <>
              {isAdmin && (
                <Link href="/admin" className="font-extrabold text-purple-700 hover:text-purple-900 transition-colors flex items-center gap-2 bg-purple-50 hover:bg-purple-100 px-4 py-1.5 rounded-full border border-purple-200 shadow-sm">
                  Admin Panel
                </Link>
              )}
              ...
              <Link href="/api/auth/signout" className="font-bold text-red-600 hover:text-red-700 transition-colors ml-4 bg-red-50 px-4 py-1.5 rounded-full">
                Sign-Out
              </Link>
            </>
          ) : (
        ...
        <div className="md:hidden absolute top-16 left-0 w-full bg-white shadow-2xl border-t border-gray-100 flex flex-col px-6 py-6 space-y-5 animate-fade-in-down">
          ...
          {session ? (
            <div className="flex flex-col space-y-5">
              {isAdmin && (
                <Link href="/admin" onClick={closeMenu} className="font-bold text-lg text-purple-700 bg-purple-50 py-3 rounded-xl border border-purple-100 text-center hover:bg-purple-100 transition-colors">
                  Enter Admin Panel
                </Link>
              )}
              ...
              <Link href="/api/auth/signout" onClick={closeMenu} className="font-bold text-lg text-center bg-red-50 text-red-600 py-3 rounded-xl border border-red-100 hover:bg-red-100 transition-colors">
                Sign-Out
              </Link>
            </div>
          ) : (
            <div className="flex flex-col space-y-4">
              <Link href="/signin" onClick={closeMenu} className="font-bold text-lg text-center bg-gray-50 text-gray-800 py-3 rounded-xl border border-gray-200 hover:bg-gray-100 transition-colors">Sign-In</Link>
              <Link href="/signup" onClick={closeMenu} className="font-bold text-lg text-center bg-cyan-600 text-white py-3 rounded-xl shadow-md hover:bg-cyan-700 transition-colors">Sign-Up</Link>
            </div>
          )}
        </div>
      )}
